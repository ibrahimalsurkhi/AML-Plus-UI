...
import { transactionService, ruleService, type TransactionCaseDetails, type TransactionCase, type PaginatedResponse, type RuleDetails, type Rule } from '@/services/api';
...
const TransactionCaseDetailsPage = () => {
  ...
  const getRuleTypeLabel = (ruleType: number) => {
    switch (ruleType) {
      case 1: return 'Transaction Monitoring';
      case 2: return 'Transaction Screening';
      case 3: return 'Financial Fraud';
      case 4: return 'Anti-Concealment';
      default: return `Type ${ruleType}`;
    }
  };

  const getActionTypeLabel = (actionType: string) => {
    switch (actionType.toLowerCase()) {
      case 'alert': return 'Generate Alert';
      case 'block': return 'Block Transaction';
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <dl className="space-y-3">
                       ...
                       <dd className="text-sm">{caseData.transactionType?.name || 'Unknown Type'}</dd>
                     </div>
                     <div>
                   ...
                   {caseData.record ? (
                     <dl className="space-y-2">
                       <div>
                         ...
                         <dd className="text-sm">{caseData.record.firstName} {caseData.record.middleName || ''} {caseData.record.lastName}</dd>
                       </div>
                       <div>
                       ...
                       <div>
                         <dt className="text-sm font-medium text-muted-foreground">Identification</dt>
               ...
               {ruleLoading ? (
                 ...
                 <div>
                   ...
                   <div className="rounded-lg bg-blue-50 border border-blue-200 px-4 py-3 text-sm font-mono text-gray-800">
                     {ruleDetails?.root ? generateRuleSummary(ruleDetails) : '[Metric] [Operator] [Value] in last [Duration] [Duration Type] for [Account Type]'}
                   </div>
                 </div>
               ) : (
                 <div className="text-center py-8">
                   <KeenIcon icon="document" style="duotone" className="text-muted-foreground text-3xl mx-auto mb-2" />
           ...
           <Card>
             ...
             <CardContent>
               {relatedCasesLoading ? (
                 ...
                 <div className="text-center py-8">
                   <KeenIcon icon="shield-tick" style="duotone" className="text-muted-foreground text-3xl mx-auto mb-2" />
