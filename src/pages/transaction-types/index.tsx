...
const TransactionTypesPage = () => {
  ...
  const [transactionTypes, setTransactionTypes] =
    useState<PaginatedResponse<TransactionType> | null>(null);
  ...
  const handleSwitchActive = async (id: number) => {
    try {
      ...
      toast({
        title: 'Error',
        description: 'Failed to update transaction type status',
        variant: 'destructive'
      });
    } finally {
      setSwitchingId(null);
              ...
              <TableBody>
                {transactionTypes?.items.map((type) => (
                  <TableRow key={type.id}>
                    ...
                    <TableCell>
                      <Button size="sm" variant="ghost" onClick={() => handleEdit(type.id)}>
                        Edit
                      </Button>
                      <Button
                        size="sm"
        ...
        <Pagination>
          <PaginationContent>
            ...
            <PaginationItem>
              <PaginationNext
                onClick={() =>
                  setPageNumber((prev) => Math.min(prev + 1, transactionTypes.totalPages))
                }
              />
            </PaginationItem>
          </PaginationContent>
